...
This is the ONLY sanctioned way docs/assets/data/*.json may change. Run
`scrape.py all` and `diff_corpus.py` first, and read the diff report.

Usage:
  python research/scripts/promote.py --dry-run     # report what would change
  python research/scripts/promote.py --apply       # write docs/assets/data/controls.json

What it does
------------
Rebuilds `controls.json` from `research/corpus/scraped/controls.json`, keeping the
shipped schema so `docs/assets/js/controls.js` needs no change:
...
def build():
    up = {c["id"]: c for c in json.loads(SCRAPED.read_text(encoding="utf-8"))}
    lo = {c["id"]: c for c in json.loads(SHIPPED.read_text(encoding="utf-8"))}
    if set(up) != set(lo):
...
def main():
    ...
    out, changes = build()
    print("%d controls, all carrying guidance" % len(out))
    for k, v in sorted(changes.items(), key=lambda x: -x[1]):
        print("  %-34s %3d" % (k, v))

    if a.apply:
        SHIPPED.write_text(json.dumps(out, indent=2, ensure_ascii=False) + "\n",
                           encoding="utf-8")
        print("\nwrote %s" % SHIPPED)
    else:
